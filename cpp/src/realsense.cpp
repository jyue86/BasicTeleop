...
void RealsenseCameraOp::start() {
  ...
  try {
    ...
    HOLOSCAN_LOG_INFO("Realsense camera started with fallback. Color: {}x{} @ {}fps, Depth: {}x{} @ {}fps", 
        color_stream.width(), color_stream.height(), color_stream.fps(),
        depth_stream.width(), depth_stream.height(), depth_stream.fps());
  }
}

void RealsenseCameraOp::stop() {
  pipeline_.stop();
  HOLOSCAN_LOG_INFO("Realsense camera stopped.");
}
...
void RealsenseCameraOp::compute(InputContext& op_input, OutputContext& op_output,
  ...
  nvidia::gxf::Handle<nvidia::gxf::VideoBuffer> depth_buffer =
      static_cast<nvidia::gxf::Entity>(depth_buffer_message)
          .add<nvidia::gxf::VideoBuffer>("depth_buffer")
          .value();
...
void RealsenseApp::compose() {
    auto allocator = make_resource<UnboundedAllocator>("allocator");
    auto realsense_camera_op = make_operator<ops::RealsenseCameraOp>(
        "realsense_camera_op",
        make_condition<PeriodicCondition>("frame_limiter", Arg("recess_period", std::string("60Hz"))),
        Arg("allocator", allocator));
    std::vector<ops::HolovizOp::InputSpec> input_spec;
    auto& color_spec = input_spec.emplace_back(
        ops::HolovizOp::InputSpec("color_buffer", ops::HolovizOp::InputType::COLOR));
    color_spec.views_.push_back(ops::HolovizOp::InputSpec::View{0.0f, 0.0f, 0.5f, 1.0f});
    auto& depth_spec = input_spec.emplace_back(
        ops::HolovizOp::InputSpec("depth_buffer", ops::HolovizOp::InputType::COLOR));
    depth_spec.views_.push_back(ops::HolovizOp::InputSpec::View{0.5f, 0.0f, 0.5f, 1.0f});

    auto visualizer = make_operator<ops::HolovizOp>(
        "holoviz",
        Arg("window_title", "Realsense Visualization"),
        Arg("width", 1280U * 2),
        Arg("height", 720U * 2),
...
int main() {
    auto app = holoscan::make_application<RealsenseApp>();
    app->run();
    return 0;
}